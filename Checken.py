        ...
        if "ban" in stoped:
            ...
            if str(message.chat.id) in files:
                ...
                if int(hour)>= 20 or int(hour) < 8: 
                    ...
                    if DEBUG == False or message.chat.id == 104932971:
                        marsh1(message)
                    else:
    ...
    try:
        if "Назад" in message.text:
            ...
            try:
                ...
                cursor = conn.cursor();
                cursor.execute("DELETE FROM 'day1' WHERE last_index = '"+num+"'")
                cursor.execute("DELETE FROM 'day2' WHERE last_index = '"+num+"'")
                cursor.execute("DELETE FROM 'day3' WHERE last_index = '"+num+"'")
                cursor.execute("DELETE FROM 'day4' WHERE last_index = '"+num+"'")
                cursor.execute("DELETE FROM 'day5' WHERE last_index = '"+num+"'")
                cursor.execute("DELETE FROM 'day6' WHERE last_index = '"+num+"'")
                cursor.execute("DELETE FROM 'day7' WHERE last_index = '"+num+"'")
                #
                z=22;
                for i in range(9):
                    cursor.execute("DELETE FROM 'day"+str(z)+"' WHERE last_index = '"+num+"'")
                    z+=1;
                #
                conn.commit();
                conn.close();
...
def send_day_orders(message,base):
    try:
        ...
        for i in range(len(data)):
            try:
                bot.send_message(channel_drivers, data[i][0],reply_markup=keyboard_send_loc(message,last_index[i][0]));
                if lat != "0":
                    bot.send_location(channel_drivers,float(lat[i][0]),float(lon[i][0]))
                save_in_history(data[i][0],lon[i][0],lat[i][0],last_index[i][0]);
                cursor.execute("DELETE FROM '"+base+"' WHERE data = '"+data[i][0]+"'")
                conn.commit();
            except:
                bot.send_message(message.chat.id, "Были отправлены не все заказы, попробуйте снова через пару минут.\n Но накладная уже готова, вот она:");
                break;
            ...
            if len(files)==0:
                ...
                for i in range(len(basket)):
                    ...
                    try:
                        ...
                        buf_append(message,"Доп. контакт: "+com)
                    except:
                        pass
                if lang == "uz":
                ...
                res = state_read(message,"buf")
                bot.send_message(message.chat.id, res,reply_markup=keyboard_basket(message))
                if lang == "ru":
...
def send(message):
    try:
        if message.text == "⬅️ Orqaga" or message.text == "⬅️ Назад":
            ...
            start(message)
        elif message.text == "✅Ha" or message.text == "✅Да":
            send_ok(message)
        else:
            start(message)
    except Exception as e:
...
def send_ok(message):
    try:
        ...
        file = open("buf/"+str(message.chat.id)+"last_index.txt","w")
        file.write(last_index)
        file.close()

        file = open("buf/"+str(message.chat.id)+"buf.txt","w")
        file.write(data)
        file.close()

        file = open("buf/"+str(message.chat.id)+"lat.txt","w")
        file.write(lat)
        file.close()

        file = open("buf/"+str(message.chat.id)+"lon.txt","w")
        file.write(lon)
        file.close()

        new_order_add(message,contact,adres,data,last_index)

        bot.send_message(channel, data, reply_markup=keyboard_accept(message))
        if lat != "0":
        ...
        if False:#DEBUG == True and message.chat.id != 104932971: 
            ...
            if call.message:
                ...
                if "no" in call.data:
                    ...
                    del_id = call.data.replace("no","")
                    
                    bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=str(message.text)+"\n\nОтклонено❌")
                    bot.send_message(del_id, "Ваш заказ был отклонен, за подробной информацией обратитесь по ссылке: _")
                
                if "accept" in call.data:
                    hour = int(current_datetime.hour)
                    del_id = call.data.replace("accept","")
                    data = call.message.text;
                    statistic_write(message,data)
                    file =open("buf/"+str(del_id)+"lat.txt","r")
                    lat = file.read();
                    file.close()

                    file =open("buf/"+str(del_id)+"lon.txt","r")
                    lon = file.read();
                    file.close()
                    save_order(message,data,lon,lat);
                    day = dayFromTxt(data);

                    bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=str(message.text)+"\n\n✅Принято✅")
                    bot.send_message(del_id,"Спасибо за доверие, Ваша заявка принята!\nДень: "+day)
                if "day" in call.data:
                ...
                if "get_location" in call.data:
                    ...
                    loc_in_channel(message,last_index);
                if "order_done" in call.data:
                    ...
                    conn = sqlite3.connect('orders.sqlite');
                    cursor = conn.cursor();
                    cursor.execute("DELETE FROM 'orders' WHERE last_index = '"+last_index+"'")
                    conn.commit();
                    conn.close();
...
#+++++++++++++функционал++++++
def loc_in_channel(message,last_index):
    conn = sqlite3.connect('orders.sqlite')
    cursor = conn.cursor()
    cursor.execute("SELECT lon FROM 'orders' WHERE last_index = '"+last_index+"'")
    lon = cursor.fetchall()[0][0];
    cursor.execute("SELECT lat FROM 'orders' WHERE last_index = '"+last_index+"'")
    lat = cursor.fetchall()[0][0];
    print("ok");
    bot.send_location(channel_drivers,float(lat),float(lon),reply_markup=keyboard_del_loc(message));
    conn.close()
...
def save_order(message,data,lon,lat):
    try:
        ...
        cursor.execute("INSERT INTO '"+base+"' VALUES((?),(?),(?),(?),(?))",(index,data,lon,lat,last_index))
        conn.commit()
        cursor.execute("SELECT id FROM '"+base+"'");
            ...
            if "Индейка" in line:
                ...
                for z in range(len(howLine)):
                    if howLine[z]=="x":
                        ...
                        howD+=howLine[z];
        bot.send_message(message.chat.id,"В базе "+base+" есть "+str(ind)+" заказов содержащих индейку\nВсего индеек в данной базе: "+str(indAll));
        if index>=MAX_ORDERS:
            conn = sqlite3.connect('db.sqlite');
            cursor = conn.cursor();
            cursor.execute("UPDATE days SET '{0}' = '❌'".format(base));

            bot.send_message(message.chat.id,"База "+base+" была заполнена на "+str(MAX_ORDERS)+" И была закрыта");
            conn.commit();
            conn.close();
        else:
            bot.send_message(message.chat.id,"В базе "+base+" теперь "+str(index)+" заказов");
...
def new_order_add(message,contact,adres,data,last_index):
    try:
        ...
        cursor.execute("INSERT INTO orders VALUES((?),(?),(?),(?),(?),(?),(?),(?),(?))",(index,status,name,tg_id,contact,adres,data,time,last_index))
        conn.commit()
        conn.close()
    except Exception as e:
        logger(message,e)
...
def statistic_write(message,text):
    try:
        conn = sqlite3.connect('db.sqlite')
        cursor = conn.cursor()
        cursor.execute("SELECT name FROM category")
        ...
        for i in range(len(strings)):
            ...
            if "№" in string:
                ...
                index = str(len(cursor.fetchall())+1);
                cursor.execute("INSERT INTO all_stat VALUES((?),(?),(?),(?),(?),(?),(?))",(index,Cat,Prod,cat_prod,saldo,time,how_many))
...
def buf_append(message,insert):
    try:
        ...
        if len(results) == 0:
            cursor.execute("INSERT INTO states VALUES('"+str(place)+"','"+str(results)+str(insert)+"')")
        else:
            cursor.execute("UPDATE states SET value = (?) WHERE key = (?)", (results1[0][0]+insert,place))
        conn.commit()
        conn.close()
    except Exception as e:
        logger(message,e)
...
def get_price(prod):
    ...
    for i in range(len(names)):
        if names[i][0] in prod:
            cursor.execute("SELECT id FROM product WHERE name=(?)",(names[i][0],));
            prod_id = cursor.fetchall()[0][0];
            break;
    cursor.execute("SELECT price FROM product WHERE id =(?)",(prod_id,));
...
def keyboard_num(message):
    try:
        ...
        print(path)
        if "Куры целые🐓" in path or "Индейка" in path:
            var = "шт";
        else:
            var = "кг";
        lang = state_read(message,"lang")
...
def keyboard_inline_days(message):
    try:
        conn = sqlite3.connect('days.sqlite')
        cursor = conn.cursor()
        cursor.execute("SELECT id FROM 'day1'")
        day1 = str(len(cursor.fetchall()));
        cursor.execute("SELECT id FROM 'day2'")
        day2 = str(len(cursor.fetchall()));
        cursor.execute("SELECT id FROM 'day3'")
        day3 = str(len(cursor.fetchall()));
        cursor.execute("SELECT id FROM 'day4'")
        day4 = str(len(cursor.fetchall()));
        cursor.execute("SELECT id FROM 'day5'")
        day5 = str(len(cursor.fetchall()));
        cursor.execute("SELECT id FROM 'day6'")
        day6 = str(len(cursor.fetchall()));
        cursor.execute("SELECT id FROM 'day7'")
        day7 = str(len(cursor.fetchall()));
        markup = types.InlineKeyboardMarkup(row_width=3)
        ...
        z=22;
        for i in range(9):
            cursor.execute("SELECT id FROM 'day{0}'".format(str(z)))
            day = str(len(cursor.fetchall()));
...
def keyboard_site2(message):
    try:
        ...
        markup = types.InlineKeyboardMarkup(row_width=3);
        if lang == "ru":
            url_button = types.InlineKeyboardButton(text="Напишите нам", url="https://t.me/Diana2727");
        else:
            url_button = types.InlineKeyboardButton(text="Bizga yozing", url="https://t.me/Diana2727");
        markup.row(url_button);
        return markup;
    except Exception as e:
        logger(message,e);
...
def keyboard_accept(message):
    try:
        markup = types.InlineKeyboardMarkup(row_width=3)
        btn1 = types.InlineKeyboardButton(text="Принять✅", callback_data=str(message.chat.id)+"accept")
        btn2 = types.InlineKeyboardButton(text="Отклонить❌", callback_data=str(message.chat.id)+"no")
        markup.row(btn1,btn2)
        return markup
    except Exception as e:
        logger(message,e)
